...
RLAPI void rlLoadAMesh(AMesh *amesh, bool dynamic);                          // Upload vertex data into GPU and provided VAO/VBO ids
RLAPI void rlDrawAMesh(AMesh amesh, Material material, Matrix transform);    // Draw a 3d mesh with material and transform
...
RLAPI void rlglDraw(void);                            // Update and Draw default buffers (lines, triangles, quads)
...
RLAPI RenderTexture2D rlLoadRenderTexture(int width, int height);         // Load a texture to be used for rendering (fbo with color and depth attachments)
